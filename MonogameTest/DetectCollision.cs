...
public class DetectCollisions : Game1
{
    ...
    //checks the collision detection over what is returned by iterating
    public static Dictionary<(ICollidable, ICollidable), typeCollision> CheckCollisions(List<(ICollidable, ICollidable)> collisionPairs)
    {
        var collisionResults = new Dictionary<(ICollidable, ICollidable), typeCollision>();

        foreach (var pair in collisionPairs)
        {
            var animated = pair.Item1;
            var other = pair.Item2;

            var collisionType = new DetectCollisions(animated.BoundingBox, other.BoundingBox);

            if (collisionType != typeCollision.None)
            {
                collisionResults.Add((animated, other), collisionType);
            }
        }
